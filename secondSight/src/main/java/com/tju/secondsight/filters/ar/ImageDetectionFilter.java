package com.tju.secondsight.filters.ar;
...
public class ImageDetectionFilter implements Filter {
	...
	public ImageDetectionFilter(final Context context,
		...
		this.mReferenceImage = Utils.loadResource(context, referenceImageResourceID,
				Highgui.CV_LOAD_IMAGE_COLOR);
		final Mat referenceImageGray =  new Mat();
		Imgproc.cvtColor(mReferenceImage,
				referenceImageGray, Imgproc.COLOR_BGR2GRAY);
	...
	public void apply(Mat src, Mat dst) {
		...
		this.mFeatureDetector.detect(mGraySrc, mSceneKeypoints);
		this.mDescriptorExtractor.compute(mGraySrc, mSceneKeypoints, mSceneDescriptors);
		this.mDescriptorMatcher.match(mReferenceDescriptors, mSceneDescriptors, mMatches);
		findSceneCorners();
		draw(src, dst);
	}
